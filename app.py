        ...
        try:
            ...
            if self.face_cascade is not None:
                ...
                if len(faces) > 0:
                    ...
                    # Fallback: use center crop of image instead of failing
                    print("⚠️ No face detected by cascade – using center crop as fallback")
                    h, w = gray.shape
                    size = min(h, w)
                    x = (w - size) // 2
                    y = (h - size) // 2
                    face_region = gray[y:y+size, x:x+size]
            else:
                # No cascade available → fall back to center crop
                h, w = gray.shape
                size = min(h, w)
                x = (w - size) // 2
                y = (h - size) // 2
...
MATCHING_THRESHOLD = 0.88   # 88% required for check-in/check-out match
...
def checkin_employee():
    ...
    try:
        ...
        # Match face against registered templates
        best_match, best_score, match_details = match_face_accurately(test_features, test_hash)
        
        print(f"🔍 Check-in matching result: best_score={best_score:.2f}, details={match_details}")
        
        if best_match:
            # Check if last action was check-in (allow multiple sessions per day)
            last_action = AttendanceLog.query.filter(
                AttendanceLog.employee_id == best_match.employee_id,
                db.func.date(AttendanceLog.timestamp) == get_ist_date()
            ).order_by(AttendanceLog.timestamp.desc()).first()
            
            if last_action and last_action.checkin_type == 'IN':
                return jsonify({
            ...
            print(f"✅ CHECK-IN: {best_match.name} ({best_score:.2f}%)")
            ...
            # No accepted match: treat as NOT REGISTERED
            if best_score > 0:
                if best_score >= REJECT_THRESHOLD * 100:
                    msg = f'Face not registered (best similarity {best_score:.2f}% < {MATCHING_THRESHOLD*100:.1f}%).'
                else:
                    msg = 'Face not registered in the system.'
            else:
                msg = 'Face not registered in the system.'

            print(f"❌ CHECK-IN FAILED (not registered): {msg} | details={match_details}")
            return jsonify({
...
def checkout_employee():
    ...
    try:
        ...
        if best_match:
            # Check if last action was check-out (allow multiple sessions per day)
            last_action = AttendanceLog.query.filter(
                AttendanceLog.employee_id == best_match.employee_id,
                db.func.date(AttendanceLog.timestamp) == get_ist_date()
            ).order_by(AttendanceLog.timestamp.desc()).first()

            if last_action and last_action.checkin_type == 'OUT':
                return jsonify({
            ...
            print(f"✅ CHECK-OUT: {best_match.name} ({best_score:.2f}%)")
            ...
            print(f"❌ CHECK-OUT FAILED (not registered): {msg}")
            return jsonify({
                'status': 'not_registered',
                'message': msg
            })
